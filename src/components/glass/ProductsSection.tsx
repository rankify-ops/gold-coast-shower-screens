  ...
  return (
    ...
    >
      <div className="lg:grid lg:grid-cols-[0.5fr_1fr]">
        <div className="hidden lg:block">
          <div className="sticky top-[71px] flex h-[calc(100svh-71px)] flex-col justify-center px-14">
            ...
            >
              Four things.
              <br />
              <span className="text-[color:var(--navy)]/45">All of them glass.</span>
            </h2>

            <ol className="mt-14 border-t border-[color:var(--navy)]/15">
              {PRODUCTS.map((p, i) => (
                <li key={p.name} className="border-b border-[color:var(--navy)]/15">
                  ...
                  >
                    {/* The number stays micro — it is an index, not a name.
                        The name is what you click, so it is set at list scale
                        rather than label scale: at 11px uppercase the four
                        ranges read as a footnote under a 50px heading. */}
                    <span
                      className={`text-[12px] font-medium uppercase tracking-[0.14em] transition-colors duration-500 ${
                        i === active
                          ? "text-[color:var(--sky)]"
                          : "text-[color:var(--navy)]/40"
                      }`}
                    >
                      {p.index}
                    </span>
                    ...
                    <span
                      aria-hidden
                      className={`ml-auto h-[2px] bg-[color:var(--sky)] transition-all duration-700 ${
        ...
        <div className="lg:border-l lg:border-[color:var(--navy)]/12">
          {PRODUCTS.map((p, i) => (
            <div
              key={p.name}
              ref={(el) => {
              ...
              className="flex min-h-svh flex-col justify-center border-b border-[color:var(--navy)]/12 px-8 py-20 last:border-b-0 lg:px-14"
            >
              <div className="flex items-center justify-between text-[11px] font-medium uppercase tracking-[0.14em] text-[color:var(--navy)]/45">
              ...
              <div className="mt-8 grid gap-10 sm:grid-cols-[1fr_0.85fr]">
                <div>
                  <p className="max-w-[36ch] text-[15px] leading-[1.65] text-[color:var(--navy)]/70">
                    {p.line}
                  </p>
                  <SmartLink
                    href="#configurator"
                ...
                */}
                <dl className="flex flex-col">
                  {p.specs.map((sp) => (
                    <div
                      key={sp.label}
                      className="flex items-baseline gap-4 border-b border-[color:var(--navy)]/12 py-3 text-[11px] font-medium uppercase tracking-[0.14em] last:border-b-0"
                    >
                      <dt className="text-[color:var(--navy)]/45">{sp.label}</dt>
                      <dd className="ml-auto text-right text-[color:var(--navy)]">{sp.value}</dd>
                    </div>
                  ))}
                </dl>
              </div>
