...
import { FrostHeader } from "@/components/pop/FrostHeader";
import { LogoRail } from "@/components/pop/LogoRail";
import { IntroSection } from "@/components/pop/IntroSection";
import { ProductsSection } from "@/components/pop/ProductsSection";
import { GallerySection } from "@/components/pop/GallerySection";
import { ReviewsSection } from "@/components/pop/ReviewsSection";
import { WhySection } from "@/components/pop/WhySection";
...
/*
 * /hero-glass-pop — the same page, rebuilt around contrast.
 *
 * A fork of /hero-glass, changed in exactly three ways, all of them measured
 * against rankify.com.au rather than guessed:
 *
 *  1. THE PAGE HAS A TONAL RANGE. /hero-glass runs #f0eeed, #ffffff and
 *     #f5f5f5 — three grounds spanning fifteen levels of brightness, so no
 *     section can pop against another. Rankify's grounds span two hundred and
 *     forty-seven, because one of them is near-black. Who-we-are is navy here,
 *     and it is what makes everything around it read as bright.
 *
 *  2. NO ALPHA-FADED INK. Thirty percent of the type on /hero-glass is navy at
 *     40-70% opacity. Alpha-fading is what makes type look switched off rather
 *     than secondary. Three solid tiers instead — see --ink / --ink-2 /
 *     --ink-3 in globals.css.
 *
 *  3. BODY COPY AT 16px, not 15.
 *
 * Everything else — layout, content, motion — is identical to /hero-glass.
 *
 * The original: fluted glass hero.
 *
...
export const metadata = {
  title: "Gold Coast Shower Screens — Fluted Glass Hero (Pop)",
...
export default function HeroGlass() {
  return (
    ...
    <main
      ...
      style={{ background: "#f0eeed", color: "var(--ink)" }}
    >
      {/* ── The plate ────────────────────────────────────────────────────
        ...
        */}
        <div className="max-w-[820px]">
          ...
          <SmartLink href="#reviews" className="group inline-flex items-center gap-3">
            ...
            />
            <span className="text-[11px] font-medium uppercase tracking-[0.14em] text-[color:var(--ink-2)]">
              <span className="text-[color:var(--ink)]">{RATING.score}</span>
          ...
          {/* Their own headline and paragraph, straight from HERO — this page
              ...
              must never ship. */}
          <h1
            className="mt-5 text-[clamp(38px,4.6vw,68px)] font-medium leading-[1.02] tracking-[-0.035em] text-[color:var(--ink)]"
          ...
          <p
            className="mt-5 max-w-[52ch] text-[clamp(17px,1.2vw,21px)] font-medium leading-[1.35] tracking-[-0.02em] text-[color:var(--ink)]/85"
            style={{ fontFamily: "var(--font-geist), ui-sans-serif, system-ui, sans-serif" }}
          >
          ...
          {/*
              ...
              not an oversight.
          */}
          <p className="mt-5 max-w-[52ch] text-[16px] leading-[1.65] text-[color:var(--ink-2)]">
            {HERO.body}
          </p>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4">
            ...
            <a
              href={SITE.phoneHref}
              className="flex items-center gap-3 border border-[color:var(--ink-3)] px-8 py-4 text-[11px] font-medium uppercase tracking-[0.14em] text-[color:var(--ink)] transition-colors duration-500 hover:border-[color:var(--navy)] hover:bg-[color:var(--navy)]/5"
            >
              <span
                aria-hidden
          ...
          */}
          <nav className="mt-8 flex flex-wrap items-center gap-x-7 gap-y-4 border-t border-[color:var(--rule)] pt-6">
            {HERO.categories.map((c) => (
              <SmartLink
                key={c.label}
                href="#products"
                className="group flex items-center gap-2.5 text-[11px] font-medium uppercase tracking-[0.14em] text-[color:var(--ink-2)] transition-colors duration-500 hover:text-[color:var(--ink)]"
              >
          ...
          <div className="mt-8 flex flex-wrap items-center gap-x-5 gap-y-4">
            ...
            />
            <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-[color:var(--ink-2)]">
              <span className="text-[color:var(--ink)]">{PROOF[0].figure}</span>{" "}
              clients
              <span className="mx-3 text-[color:var(--ink-3)]">/</span>
              <span className="text-[color:var(--ink)]">{PROOF[1].figure}</span>{" "}
              screens
            </p>
            ...
            <p className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.14em]">
              ...
              <span className="text-[color:var(--ink)]">Australian</span>
              <span className="text-[color:var(--ink-2)]">owned and operated</span>
            </p>
          </div>
        </div>
      </div>
      ...
      */}
      <div className="relative z-10 flex items-end justify-end px-8 pb-7 lg:px-14">
        <p className="hidden text-[11px] font-medium uppercase tracking-[0.14em] text-[color:var(--ink-3)] sm:block">
          {SITE.licence}
